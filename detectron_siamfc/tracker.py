...
from detectron import DetectronHandler
...
class bbox():
    ...
    def init_box(self, frame):
        ...
        output of __ is already one-based
        """
...
class Tracker():
    ...
    def __init__(self, model_path='siamfc/models/siamfc_pretrained.pth', title='', gpu_id=0):
        ...
        self.detectronHandler = DetectronHandler()
    ...
    def detect(self, idx):
        ...
        # yolo_output = yolo(self.filenames[idx], self.model, self.inp_dim)
        yolo_output = self.detectronHandler.detect(self.filenames[idx])
        print(yolo_output)
        for x in yolo_output:
            print( x[0][2] * x[0][3] )

        person_detections = list(set(
            filter(lambda x: x[1] == 'person', yolo_output ) ))
    ...
    def track(self, video_dir):
        ...
        if self.title == '':
            ...
            self.title = video_dir_list[-1]

            if video_dir_list[-1] == "":
                video_dir_list.remove("")
            
            # if "MOT16" in video_dir_list:
                # self.title = video_dir_list[len(video_dir_list) - 1]
            # starting tracking

        
        # starting tracking        
        framesPath = 'output/' + self.title + '/'
        ...
        for idx, frame in enumerate(frames):
            ...
            cv2.imwrite(framesPath + f'det_{idx}.jpg', frame)
            # cv2.waitKey(30)
